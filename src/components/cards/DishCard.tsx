  ...
  return (
    <Card className="overflow-hidden p-0">
      <Link to={`/consumer/dishes/${dish.dishId}`}>
        <img alt={dish.name} className="h-48 w-full object-cover" src={dish.imageURL || fallbackImage} />
      </Link>
      <div className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <Link to={`/consumer/dishes/${dish.dishId}`} className="text-lg font-bold text-ink hover:text-emerald">
              {dish.name}
            </Link>
            <p className="mt-1 text-sm text-muted">{dish.cookerName}</p>
          </div>
          <span className="text-lg font-extrabold text-clay">${dish.price.toFixed(2)}</span>
        </div>
